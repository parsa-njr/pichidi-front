...
import {
  User, Phone, Lock, LogOut, Camera, Loader2,
...
export default function CustomerProfilePage() {
  ...
  const fetchProfile = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiClient.get("/api/v1/customer/profile");
      const d = res.data?.customer;
      setProfile({ name: d?.name ?? "", phone: d?.phone ?? "", profileImage: d?.profileImage });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await apiClient.put("/api/v1/customer/profile", formData, {
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 pb-24">
        ...
        <div className="flex flex-col items-center mb-6">
          <div className="relative">
            ...
            <button className="absolute bottom-0 left-0 w-8 h-8 bg-primary rounded-full flex items-center justify-center border-2 border-white shadow">
              <Camera className="w-3.5 h-3.5 text-white" />
            </button>
          </div>
          <p className="text-xs text-gray-400 mt-2">روی عکس کلیک کنید تا تغییر دهید</p>
        </div>
